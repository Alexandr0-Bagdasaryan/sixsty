...
<body>
    <div>
        <h1>Форма,на которую приятно смотреть и приятно заполнять...</h1>
            ...
            ?>
              ...
              foreach($arr as $el)
                if ($el == 4)
                  print 'selected';
            ?>
              >Супер-Гравитация</option>
            </select>
          </label>

          <p id="bio">Биография</p>
          <label>
            <textarea placeholder="Расскажите о себе,нам очень интересно(нет)" name="biography" rows="6" cols="60"  <?php if ($errors['biography']) {print 'class="error"';} ?> ><?php print $values['biography'];?></textarea>
          </label>
          <br>
